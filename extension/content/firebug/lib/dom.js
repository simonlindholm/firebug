...
Dom.isDOMMember = function(object, propName)
{
    try
    {
        ...
        var isNative = Cu.isXrayWrapper(wrappedObject);
        if (isNative && propName in wrappedObject)
            return true;

        if (object instanceof Window && propName in WindowProps)
            return true;
        ...
        {
            // Treat "length" the same as indices.
            if (propName === "length" || !(propName in []))
                return false;

            // In Fx28+ (since bug 933681), compare the property with the default one
            // for added safety.
            if ("Array" in global)
                return Wrapper.wrapObject(object[propName]) === global.Array.prototype[propName];
            return true;
        }

        // Check for things from Object.prototype.
        if (propName in {})
        {
            // In versions before Fx28 we play it safe and only exclude Object.prototype
            // properties from natives. (We mustn't hide e.g. custom toString methods).
            if ("Object" in global)
                return Wrapper.wrapObject(object[propName]) === global.Object.prototype[propName];
            else
                return isNative;
        }
    }
    ...
    {
        // We don't know, but let's be safe and fall through to returning false.
    }
    return false;
};
...
Dom.isDOMConstant = function(object, name)
{
    ...
    var isNative = Cu.isXrayWrapper(Wrapper.wrapObject(object));
    return isNative;
};
